...
# Move to docker-compose home dir
cd ${HOME_DIR}
...
docker-compose exec --no-TTY ${MARIA_DB_DOCKER_COMPOSE_SERVICE} mysqldump -uroot -p${MYSQL_ROOT_PASSWORD} --all-databases > ${BACKUP_DIR}/matomo-db-dump.sql 2>&1 | tee -a $LOG
